...
VOID GetArchiveItemFromPath(LPCTSTR strDirPath, CObjectVector<CDirItem> &dirItems)
{
	...
	bool bret = TEST.FindFirst(fas2fs(dirPath), fi);
	if (!bret)
	{
		return;
	}
	while (TEST.FindNext(fi))
	{
		...
		di.ATime = fi.ATime;
		di.MTime = fi.MTime;
		di.Name = fs2us(fi.Name); //MultiByteToUnicodeString(fs2us(fi.Name));
		di.FullPath = MultiByteToUnicodeString(strDirPath);
		di.FullPath += MultiByteToUnicodeString("\\");
		di.FullPath += fs2us(fi.Name); //MultiByteToUnicodeString(fi.Name);
		
		dirItems.Add(di);
		GetArchiveItemFromPath(fs2fas(di.FullPath), dirItems);
	}
}
...
int MY_CDECL main(int numArgs, const char *args[])
{
  ...
  if (c == 'a')
  {
	...
	GetArchiveItemFromPath(args[3], ItemList);
...
//     }
	...
	updateCallbackSpec->Init(&ItemList);
    ...
    */
	 ...
	 HRESULT result = outArchive->UpdateItems(outFileStream, ItemList.Size(), updateCallback);
  ...
  else
  {
    ...
    else
    {
      ...
      extractCallbackSpec->Init(archive, FTEXT("C:\\Users\\neo\\Desktop\\7z1604-src\\CPP\\7zip\\UI\\Client7z\\Debug\\Test")); // second parameter is output folder path
